...
namespace DiagramEditor.ViewModels
{
    ...
    public class MainWindowViewModel : ViewModelBase
    {
            ...
            panel.PointerReleased += (object? sender, PointerReleasedEventArgs e) =>
            {
                if (e.Source != null && e.Source is Control @control)
                {
                    ...
                    if (map.tap_mode == 2 && map.tapped_item != null)
                    {
                        ...
                        menu = new AddShape { DataContext = this, Title = "Редактирование диаграммы" };
                        menu.ShowDialog(mw);
                    }
                }
            };
        ...
        {
            if (entity is not Dictionary<string, object> @dict) 
            { 
                Log.Write("General: Ожидался словарь, вместо " + entity.GetType().Name); 
                return; 
            }

            @dict.TryGetValue("name", out var value);
            name = value is not string @str ? "---" : @str;
        ...
        private void FuncApply()
        {
            ...
            foreach (var attr in attributes) 
                arr.Add(new(attr.ToString()));

            foreach (var meth in methods) 
                arr2.Add(new(meth.ToString()));
