...
namespace DiagramEditor.Models
{
    public class Mapper
    {
        ...
        {
            if ((string?)item.Tag == "marker" && marker_parent != null)
        ...
        public void Press(Control item, Point pos)
        {
            ...
            switch (mode)
            {
                ...
                case 6:
                    if (item is not ArrowFactory @arrow)
                        break;
        ...
        public void FixItem(ref Control res, Point pos, IEnumerable<ILogical> items)
        {
            foreach (var logic in items)
            {
                ...
                if (tb != null && new Rect(tb.Value.Clip.TopLeft,
                    new Size()).Sum(item.Bounds).Contains(pos)
                    && (string?)item.Tag != "arrow")
                    res = item;
        ...
        public void Move(Control item, Point pos)
        {
            if (mode == 3 || mode == 6)
            {
                ...
                FixItem(ref item, pos + new Point(0, 32), items);
            }
            ...
            if (mode == 6)
                delete_arrow = (string?)item.Tag == "deleter";
            ...
            if (Math.Pow(delta.X, 2) + Math.Pow(delta.Y, 2) > 9)
                tapped = false;
        ...
        public JoinedItems? new_join;
        public int tap_mode;
        public DiagramItem? tapped_item;
        ...
        public void Export(string type, Control canv)
        {
            ...
            {
                var m = item.Margin;
                var d = new object[] { item.entity,(int)m.Left,
                                       (int)m.Top, (int)item.Width,
                                       (int)item.Height };
            ...
            foreach (var item in items)
                joins.Add(item.ExportJoins(di_to_num));
            ...
            switch (type)
            {
                case "JSON":
                    ...
                    if (json == null)
                    {
                        Log.Write("Не удалось экспортировать в Export.json:");
                        return;
                    }
                ...
                case "XML":
                    ...
                    if (xml == null)
                    {
                        Log.Write("Не удалось экспортировать в Export.xml :");
                        return;
                    }
                ...
                case "YAML":
                    ...
                    if (yaml == null)
                    {
                        Log.Write("Не удалось экспортировать в Export.yaml :");
                        return;
                    }
        ...
        public DiagramItem[]? Import(string type, object? content = null)
        {
            ...
            if (content is not Dictionary<string, object> @dict)
            {
                Log.Write("В начале " + name + " не словарь");
                return null;
            }

            if (!@dict.TryGetValue("items", out var value))
            {
                Log.Write("В корне необнаружен items");
                return null;
            }

            if (value is not List<object> @itemz)
            {
                Log.Write("items не того типа");
                return null;
            }

            if (!@dict.TryGetValue("joins", out var value2))
            {
                Log.Write("В корне необнаружен joins");
                return null;
            }

            if (value2 is not List<object> @joins)
            {
                Log.Write("joins не того типа");
                return null;
            }

            if (items.Count > 0)
            {
                var canv = (Canvas)(items[0].Parent ?? throw new Exception("What?"));

                foreach (var child in canv.Children.Cast<Control>().ToList())
                {
                    var tag = (string?)child.Tag;

                    if (tag != "marker" && tag != "marker2")
            ...
            {
                if (obj is not List<object> @item)
                {
                    Log.Write("Один из элементов не того типа");
                    continue;
                }

                if (@item.Count != 5 ||
                    @item[1] is not int @x || @item[2] is not int @y ||
                    @item[3] is not int @w || @item[4] is not int @h)
                {
                    Log.Write("Содержимое списка элемента ошибочно");
                    continue;
                }
            ...
            {
                if (obj is not List<object> @join)
                {
                    Log.Write("Одно из соединений не того типа");
                    continue;
                }

                if (@join.Count != 7 || @join[0] is not int @num_a ||
                    @join[1] is not int @bord_a || @join[2].ToDouble() is not double @d_a ||
                    @join[3] is not int @num_b || @join[4] is not int @bord_b ||
                    @join[5].ToDouble() is not double @d_b || @join[6] is not int @t)
                {
                    Log.Write("Содержимое списка соединения ошибочно");
                    continue;
                }

                var newy = new JoinedItems(new(items_arr[@num_a], 0, 0, 0, @bord_a, @d_a),
                                           new(items_arr[@num_b], 0, 0, 0, @bord_b, @d_b));
                newy.line.Type = @t;
                joinz.Add(newy);
            }
        ...
        private DiagramItem[]? GrandImport(string data)
        {
            ...
            try { content = Utils.Json2obj(data); }
            catch { }

            if (content != null)
                return Import("JSON", content);

            try { content = Utils.Xml2obj(data); }
            catch { }

            if (content != null)
                return Import("XML", content);

            Log.Write("Не удалось разпознать тип данных : Нужен JSON | XML | YAML");
