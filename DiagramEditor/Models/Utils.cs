...
namespace DiagramEditor.Models
{
    public static class Utils
    {
        ...
        private static bool IsComposite(object? obj)
        {
            ...
            if (obj is List<object?> || obj is Dictionary<string, object?> || obj is not JsonElement @item)
                return true;
        ...
        private static string Dict2XML(Dictionary<string, object?> dict, string level)
        {
            ...
            foreach (var entry in dict)
                if (IsComposite(entry.Value))
                    ...
                    attrs.Append(" " + entry.Key + "=\"" + ToXMLHandler(entry.Value, "{err}") + "\"");

            if (items.Length == 0)
                return level + "<Dict" + attrs.ToString() + "/>";
        ...
        private static string List2XML(List<object?> list, string level)
        {
            ...
            foreach (var entry in list)
            {
                if (IsComposite(entry))
                    items.Append(ToXMLHandler(entry, level + "\t"));
                else
                    attrs.Append($" _{num}='" + ToXMLHandler(entry, "{err}") + "'");

                num++;
            }

            if (items.Length == 0)
                return level + "<List" + attrs.ToString() + "/>";
        ...
        private static string ToXMLHandler(object? obj, string level)
        {
            ...
            if (obj is List<object?> @list)
                return List2XML(@list, level);

            if (obj is Dictionary<string, object?> @dict)
                return Dict2XML(@dict, level);
        ...
        public static string? Json2xml(string json)
        {
            ...
            if (json[0] == '[')
                ...
                (json[0] == '{') data = JsonSerializer.Deserialize<Dictionary<string, object?>>(json);
            else
                return null;
        ...
        private static string ToYAMLHandler(object? obj, string level)
        {
            ...
            if (obj is List<object?> @list)
                return List2YAML(@list, level);

            if (obj is Dictionary<string, object?> @dict)
                return Dict2YAML(@dict, level);
                ...
                case '-':
                    {
                        ...
                        while (true)
                        {
                            ...
                            else
                            {
                                ...
                                if (layer != layer3)
                                {
                                    if (layer3.Length > layer.Length)
                                        throw new Exception("Ожидался элемент списка вместо подъёма");
                                    if (!layer.StartsWith(layer3))
                                        throw new Exception("Странность в упавшем layer'е");
                            ...
                            if (yaml[pos++] != '-')
                                throw new Exception("Ожидалось '-' в следующем элементе списка");
                            ...
                            if (c == ' ')
                            {
                                ...
                                throw new Exception("После '-' ожидалось ' ', либо '\n'");
                            ...
                            {
                                if (!layer.StartsWith(layer2))
                                    throw new Exception("Странность в упавшем layer'е");

                                YAML_Log("Падение"); pos = saved_pos; break;
                            }

                            if (!layer2.StartsWith(layer))
                                throw new Exception("Странность в следующем layer'е");
                            ...
                            else if (c == '\n')
                            {
                                ...
                                throw new Exception("Здесь не может быть подъёма");
                        }
                ...
                default:
                    {
                        ...
                        while (true)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    if (layer3.Length > layer.Length)
                                        throw new Exception("Ожидался элемент словаря вместо подъёма");
                            ...
                            if (yaml[pos++] != ':')
                                throw new Exception("После ключа ожидалось ':'");
        ...
        public static string Yaml2json(string yaml)
        {
            ...
            {
                Log.Write("Ошибка YAML парсера: " + e);
                throw;
            }
        }
