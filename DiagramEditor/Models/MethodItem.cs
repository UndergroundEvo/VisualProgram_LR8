...
namespace DiagramEditor.Models
{
    public class MethodItem
    {
        ...
        public MethodItem(MainWindowViewModel mwvm, object entity) : this(mwvm)
        {
            if (entity is not Dictionary<string, object> @dict)
            {
                Log.Write("MethodItem: Ожидался словарь, вместо " + entity.GetType().Name);
                return;
            }
            ...
            if (value5 is IEnumerable<object> @propz)
                foreach (var prop in @propz)
                    props.Add(new PropertyItem(this, prop));
        }
    }
}
